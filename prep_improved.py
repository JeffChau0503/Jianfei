...
type_dir2 = 'E:/imageset/calcite0716pic/'  # ---------------------------------------更改3 文件保存路径 注意: 最后的"/"不能省
...
for i in range(m):
    ...
    '''
    save_path = os.path.join(type_dir1, "0_" + str(i).zfill(5) + '.jpg')
...
for i in range(m):
    ...
    plt.yticks([])
    plt.axis('off')  # 删掉坐标轴
    plt.savefig(type_dir2 + "0_" + str(i).zfill(5) + '.jpg')
    plt.clf()
